...
use super::journal::{
    ...
    JournalDirectoryV2, JournalSnapshotV2, ManagedChildKindV2, ObjectIdentityV2, PreimageV2,
};
            ...
            observe_regular_child(
                ...
                match entry.preimage() {
                    PreimageV2::Regular { exact } => {
                        exact.state().byte_len().max(entry.planned().byte_len())
                    }
                    PreimageV2::Absent => entry.planned().byte_len(),
                },
                journal_path,
            )?,
            journal_path,
...
fn observe_regular_child(
    ...
    match fs.observe_regular_file_bounded(&parent, Path::new(name), diagnostic_path, max_bytes) {
...
fn observe_logical_directory(
    ...
    let object = exact_directory(&observation)
        ...
        .map_err(|error| model_error(journal_path, &diagnostic_path, error.reason()))?;
    let inventory = if inventory_required {
        let file_read_limits = managed_file_read_limits(snapshot, model, journal_path)?;
        Some(capture_inventory(
            fs,
            endpoint,
            &observation,
            &diagnostic_path,
...
fn managed_file_read_limits(
    snapshot: &JournalSnapshotV2,
    directory: &JournalDirectoryV2,
    journal_path: &Path,
) -> Result<BTreeMap<String, u64>, CodegenError> {
    let mut limits = BTreeMap::new();
    for entry in snapshot.entries() {
        if immediate_parent(entry.logical_path()) != directory.logical_path() {
            continue;
        }
        let target_max = match entry.preimage() {
            PreimageV2::Regular { exact } => {
                exact.state().byte_len().max(entry.planned().byte_len())
            }
            PreimageV2::Absent => entry.planned().byte_len(),
        };
        insert_managed_file_read_limit(
            &mut limits,
            leaf_name(entry.logical_path()),
            target_max,
            journal_path,
        ...
        if let Some(backup) = entry.backup() {
            let backup_max = match entry.preimage() {
                PreimageV2::Regular { exact } => exact.state().byte_len(),
                PreimageV2::Absent => 0,
            };
            insert_managed_file_read_limit(&mut limits, backup.name(), backup_max, journal_path)?;
        }
    }
    ...
    let declared = directory
        .managed_children()
        .iter()
        .filter(|child| child.kind() == ManagedChildKindV2::File)
        .map(|child| child.name().to_owned())
        .collect::<BTreeSet<_>>();
    if limits.keys().cloned().collect::<BTreeSet<_>>() != declared {
        return Err(recovery_blocked(
            journal_path,
            format!(
                "created directory {} has a managed-file declaration that does not match the bounded recovery cohort",
                directory.logical_path()
            ),
        ));
    }
...
fn insert_managed_file_read_limit(
    limits: &mut BTreeMap<String, u64>,
    name: &str,
    max_bytes: u64,
    journal_path: &Path,
) -> Result<(), CodegenError> {
    if limits.insert(name.to_owned(), max_bytes).is_some() {
        return Err(recovery_blocked(
            journal_path,
            format!("managed recovery file name {name} is ambiguous"),
        ));
    }
    Ok(())
}
...
fn capture_inventory_child(
    ...
    match entry.kind {
        ExactDirectoryEntryKind::RegularFile => {
            if let Some(max_bytes) = declared_max_bytes {
                let read = fs
                    .read_regular_file_exact(parent, Path::new(name), diagnostic_path, max_bytes)
                    .map_err(|error| {
                        observation_error(
                ...
                return exact_file(&read.observation)
                    .map(ExactRecoveryObject::File)
                    .map_err(|error| model_error(journal_path, diagnostic_path, error.reason()));
            }

            // Foreign names are represented exactly enough to reject the
            // inventory, but their contents are never opened or hashed.
            let observed = fs
            ...
            require_file_metadata_entry_matches(entry, &observed, journal_path, diagnostic_path)?;
            file_metadata_object(&observed, journal_path, diagnostic_path)
        }
...
fn require_file_metadata_entry_matches(
    entry: &ExactDirectoryEntry,
    observed: &ExactFileMetadataObservation,
    journal_path: &Path,
    diagnostic_path: &Path,
) -> Result<(), CodegenError> {
    if entry.kind == ExactDirectoryEntryKind::RegularFile
        ...
        && entry.link_count == observed.link_count
    {
        ...
        Err(recovery_blocked(
            journal_path,
            format!(
                "inventory file {} changed between exact directory inventory and no-follow metadata observation",
                diagnostic_path.display()
            ),
        ))
    }
