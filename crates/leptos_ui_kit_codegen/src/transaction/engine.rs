...
use super::fs::{
    ...
    ExactFileObservation, ExactObjectIdentity, ExactRelocationSource, FsOps, HardLinkEndpoint,
    ParentSyncKind,
};
use super::journal::{
    ...
    PlannedFileStateV2, PreimageV2, PreparationObservationV2, PreparationPendingIntentV2,
    PreparationPlacementIntentV2, PresenceV2, ReplacementObservationV2, RollbackIntentV2,
    Sha256Digest,
};
...
pub(crate) fn apply_exact_transaction(
    context: &PlanningContext,
    lock: &WriteLock,
    files: &[PlannedFile],
    changes: &[ChangeRecord],
...
fn execute_ordered(
    ...
    match execution {
        ...
        Err(original) => {
            ...
            drop(store);
            let recovery =
                super::recovery::recover_pending_locked_with_runtime(context, lock, &build_runtime);
...
fn build_entries(
    ...
    ordered
        ...
        .map(|ordered| {
            ...
            let target = observe_target(context, fs, path)?;
...
fn prepare_directories(
    ...
    for directory in directories.iter().filter(|directory| {
        ...
        let owner_exact =
            exact_directory(&owner_observation).map_err(model_error_at(&owner_path))?;
...
fn observe_file_child_optional(
    ...
    match fs.observe_regular_file(parent, name, path) {
...
#[allow(
    ...
    reason = "the placement boundary receives each independently authenticated capability and exact state"
)]
fn place_file_owner(
    context: &PlanningContext,
    lock: &WriteLock,
    ...
    let owner =
        observe_file_child_optional(runtime.fs(), &workspace, Path::new(owner_name), &owner_path)?
            .ok_or_else(|| recovery_missing(logical_path, context))?;
    if !file_matches(&owner, &expected_owner, Some(1))
        || observe_file_child_optional(runtime.fs(), &parent, Path::new(placed_name), &target_path)?
            .is_some()
    {
    ...
    sync_transaction_workspace(context, lock, store, &owner_path)?;
    if observe_file_child_optional(runtime.fs(), &workspace, Path::new(owner_name), &owner_path)?
        .is_some()
    {
        return third_state(logical_path, context);
    }
        ...
        match target {
            ...
            | super::journal::CleanupTargetV2::PlacedBackup { ordinal } => {
                ...
                let observed = runtime
                    .fs()
                    .observe_regular_file(&parent, Path::new(name), path)
                    .map_err(|source| {
...
/// filesystem mutation followed by its completion publication. It never
/// enters the unbounded normal-execution rollback or cleanup loops.
pub(super) fn recover_loaded_transaction_step(
    ...
    match preflight {
        ...
        RecoveryPreflightV2::PendingPlacement {
            ...
            reconcile_pending_placement(context, lock, &mut store, intent, artifact, world)?;
            Ok(BoundedRecoveryStep::Advanced)
        }
        ...
        RecoveryPreflightV2::PendingRollback { ordinal, world } => {
            ...
            let _ = world;
            complete_pending_rollback(context, lock, &mut store, intent)?;
            Ok(BoundedRecoveryStep::Advanced)
        }
        RecoveryPreflightV2::PendingCleanup { target, world } => {
            ...
            let _ = world;
            let parent_after = execute_cleanup_intent(context, lock, &store, outcome, &intent)?;
            let successor = store
                .snapshot()
                .complete_cleanup(
                    store.records().last().expect("record exists").clone(),
...
fn recover_rollback_cursor_step(
    ...
    match (entry.action(), entry.current_target(), entry.preimage()) {
        (EntryActionV2::Create, PresenceV2::Missing, PreimageV2::Absent) => {
            if observe_target(context, store.runtime().fs(), entry.logical_path())?.is_some() {
                return third_state(entry.logical_path(), context);
            }
            let successor = store
                .snapshot()
        ...
        (
            ...
            let actual = observe_target(context, store.runtime().fs(), entry.logical_path())?
    ...
    match binding.object() {
        OwnedResidualObjectV2::File(expected) => {
            let expected_observation = ExactFileMetadataObservation {
                identity: ExactObjectIdentity::from_parts(
                    expected.identity().namespace_bytes(),
                    expected.identity().object_bytes(),
                ),
        ...
        OwnedResidualObjectV2::Directory(expected) => {
            ...
            store
                ...
                .map_err(|source| {
                    transaction_io(
                        "discard owned directory residual",
                        &format!("owner ordinal {}", ordinal.get()),
                        &owner_path,
                        source,
                    )
...
fn require_discarded_owner_absent(
    ...
    match binding.object() {
        OwnedResidualObjectV2::File(expected) => {
            match store.runtime().fs().observe_regular_file_metadata(
                &workspace,
                Path::new(name),
                &owner_path,
                expected.byte_len(),
        ...
        OwnedResidualObjectV2::Directory(expected) => {
            match store.runtime().fs().open_directory_exact(
                &workspace,
                Path::new(name),
                &owner_path,
                expected.exact().mode().posix_mode().unwrap_or(0o755),
                ...
                Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(()),
                Ok(_) => third_state("discarded directory owner residual", context),
                Err(source) => Err(transaction_io(
                    "prove discarded directory owner absence",
                    name,
                    &owner_path,
                    source,
                )),
...
fn reconcile_unrecorded_replacement(
    ...
    let target = observe_target(context, store.runtime().fs(), entry.logical_path())?;
    match entry.action() {
        EntryActionV2::Create => {
            let Some(target) = target else {
                return Ok(());
            };
            let stage_exact = entry.stage().current().as_present().ok_or_else(|| {
                CodegenError::RecoveryRequired {
                    journal_path: context.project_root().join(entry.logical_path()),
                    reason: "unrecorded created target has no exact stage authority".to_owned(),
                }
            })?;
            let parent_path = immediate_parent(entry.logical_path()).unwrap_or("");
            ...
            let stage = store
                .runtime()
                .fs()
                .observe_regular_file(&parent, Path::new(entry.stage().name()), &stage_path)
                .map_err(|source| {
                    transaction_io(
                        "inspect recovery stage",
                        entry.logical_path(),
                        &stage_path,
                        source,
                    )
            ...
            let successor = store
                ...
                .record_replacement_completion(
                    store.records().last().expect("record exists").clone(),
                    ReplacementObservationV2::new(
                        exact_file(&target).map_err(model_error_at(entry.logical_path()))?,
                        PresenceV2::Present(
                            exact_file(&stage).map_err(model_error_at(&stage_path))?,
                        ),
                    ),
                )
                .map_err(model_error_at(entry.logical_path()))?;
        ...
        EntryActionV2::Replace => {
            let preimage = entry.preimage().as_exact().expect("replace preimage");
            let Some(target) = target else {
                return third_state(entry.logical_path(), context);
            };
            if file_matches(&target, preimage, None) {
                return Ok(());
            }
            let stage_exact = entry.stage().current().as_present().ok_or_else(|| {
                CodegenError::RecoveryRequired {
            ...
            let stage_path = context
                ...
                .join(parent_path)
                .join(entry.stage().name());
            match store.runtime().fs().observe_regular_file(
                &parent,
...
fn complete_pending_rollback(
    ...
    match &intent {
        RollbackIntentV2::RemoveCreatedTarget {
            ...
            let target = match store.runtime().fs().observe_regular_file(
                &parent,
        ...
        RollbackIntentV2::RestoreBackup {
            ...
            let backup_live = match store.runtime().fs().observe_regular_file(
                &parent,
    ...
    match intent {
        CleanupIntentV2::RemoveFile { target, expected } => {
            ...
            match store.runtime().fs().observe_regular_file_bounded(
                ...
                Ok(observed) if file_matches(&observed, expected, None) => {
        ...
        CleanupIntentV2::RemoveDirectory {
            ...
            match observe_directory_child(store.runtime().fs(), &parent, Path::new(name), &path)? {
                Some(observed) if directory_matches(&observed, expected) => {
                    let opened = store
                        .runtime()
                        .fs()
                        .open_directory_exact(
