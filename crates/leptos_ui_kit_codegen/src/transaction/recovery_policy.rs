...
pub(super) fn preflight_recovery_world(
    ...
    match snapshot.phase() {
        JournalPhaseV2::Preparing {
            ...
            PreparationPendingIntentV2::CreateOwner(intent) => classify_pending_owner_creation(
                snapshot,
                *completed as usize,
                observed,
                intent,
...
fn classify_pending_owner_creation(
            ...
            .is_some_and(BTreeMap::is_empty)
    {
        return Err(recovery_blocked(
            journal_path,
            "pending CreateOwner directory residual lacks a bounded exact-empty inventory",
        ));
    }
    let object = match (intent.artifact(), observed.object(key)) {
        ...
        _ => {
            return Err(recovery_blocked(
                journal_path,
                "pending CreateOwner residual has the wrong kind or observation form",
            ));
        }
    };
    ...
    if !allowed.same_objects(observed) {
        return Err(recovery_blocked(
            journal_path,
            "pending CreateOwner residual is mixed with another cohort change",
        ));
    }
...
fn classify_pending_owner_discard(
            ...
            .is_some_and(BTreeMap::is_empty)
    {
        return Err(recovery_blocked(
            journal_path,
            "pending DiscardOwner directory residual lacks a bounded exact-empty inventory",
        ));
    }
    let mut allowed = exact_snapshot_world(snapshot);
    allowed
    ...
    if !allowed.same_objects(observed) {
        return Err(recovery_blocked(
            journal_path,
            "pending DiscardOwner differs from its exact residual-before world",
        ));
    }
    Ok(RecoveryPreflightV2::PendingOwnerDiscard {
...
fn observed_directory_presence(
    ...
    match world.object(key) {
        ...
        Some(ExactRecoveryObject::DirectoryMetadata(_)) => Err(recovery_blocked(
            journal_path,
            "directory recovery object was captured as residual metadata outside CreateOwner/DiscardOwner",
        )),
        None => Err(recovery_blocked(
            journal_path,
...
fn observed_file_presence(
    ...
    match observed.object(key) {
        ...
        Some(ExactRecoveryObject::FileMetadata(_)) => Err(recovery_blocked(
            journal_path,
            "file recovery object was captured as residual metadata outside CreateOwner/DiscardOwner",
        )),
        None => Err(recovery_blocked(
            journal_path,
